...
class Graph():
    def __init__(self, d, is_directed, workers, bfs_workers=None, until_layer=None, in_degrees=None, out_degrees=None,
                 embedding_vertices=None, density=None):
        ...
        logging.info('density - Number of edges: {}'.format(self.density))
...
def load_edgelist(file_, directed=False, weighted=False):
        ...
        for l in f:
            if len(l.strip().split()[:2]) > 1:
                if weighted:
                    ...
                    y = int(y)
                    """
                        for brain network sum neg and pos density count 
                    """
                    if w >= 0:
                        pos_density[x] = pos_density.get(x, 0) + w
                        # out_degrees[x] = out_degrees.get(x, 0) + w
                    elif w < 0:
                        neg_density[x] = neg_density.get(x, 0) + w
                else:
                    x, y = l.strip().split()[:2]
                    w = 1
    ...
    count S star
    """
    key_pos_val = pos_density.keys()
    key_neg_val = neg_density.keys()
    pp = set(key_pos_val + key_neg_val)
    val_density = {}
    for item in pp:
        pos = pos_density.get(item, 0)
        neg = abs(neg_density.get(item, 0))
        val_density[item] = pos - (neg / (pos + neg)) * neg
    return skeleton, in_degrees, out_degrees, val_density
